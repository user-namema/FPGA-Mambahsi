...
[实验清单与逐项命令](docs/EXPERIMENTS.md) 包含 19 个入口：01–03 是 FP32/结构/共享 A，04–07 是量化选型，08–12 是论文旧版 eval8 与误差/GPU实验，13–16 是新版 eval1、dt 诊断与功耗，17–19 是回放与 batch 定位。
...
新 eval1 模型的 40 份 FPGA 报告已独立归档，保存 QAT 预测全部 100% 复现。UP 功耗包含 7 种 batch × 2 种范围 × 3 次重复，测量条件为保留 Xorg 显示后台、检查时点无其他计算进程。四数据集旧 FP32 测速原始 JSON、dt 诊断输出和 N0–N3 软件对照仍未收到；见 [本次补件与精确回放说明](docs/SERVER_UPDATE_20260923.md)。
