...
SIMULATOR_BUILD = "single-qat-source-d-path-shared-u-20260914"
...
def simulate_mamba_core_int8(
    ...
    if numeric.error_source == 'all':
        y_ssm_int8 = requantize_int8(
            y_ssm_q24.transpose(1, 2).contiguous(), y_q24_scale, s_out_proj_input)
    else:
        # Diagnostic counterfactual, not a bit-true RTL path.
        y_ssm_int8 = round_half_away_from_zero(
            y_ssm_q24.transpose(1, 2) / s_out_proj_input.double()
        ).clamp(INT8_MIN, INT8_MAX).to(torch.int8)
...
def rtl_integer_bilinear_argmax(logits):
    """Match up_logits_to_rows.sv: signed INT8 [C,4,4] -> UINT8 [16,16].

    align_corners=True gives coordinates x/5, y/5. Keep the common
    denominator 25 implicit: no floating point, division, rounding or clamp.
    Classes are ordered by ID; np.argmax returns the first equal maximum,
    matching the RTL's ascending scan with a strict score > best comparison.
    Returns labels, exact score numerators, and a multiple-winner mask.
    """
    q = np.asarray(logits)
    if q.ndim != 3 or q.shape[1:] != (4, 4) or not 1 <= q.shape[0] <= 256:
        raise ValueError("RTL interpolation requires logits shaped [C,4,4], 1<=C<=256")
    if not np.issubdtype(q.dtype, np.signedinteger):
        raise TypeError("RTL logits must be signed integer codes, not floats or UINT8 bytes")
    if np.any(q < -128) or np.any(q > 127):
        raise ValueError("RTL logits exceed signed INT8 range")
    q = q.astype(np.int64, copy=False)
    scores = np.empty((q.shape[0], 16, 16), dtype=np.int64)
    for y in range(16):
        by = min(y // 5, 2)
        fy = y - 5 * by
        for x in range(16):
            bx = min(x // 5, 2)
            fx = x - 5 * bx
            h0 = (5 - fx) * q[:, by, bx] + fx * q[:, by, bx + 1]
            h1 = (5 - fx) * q[:, by + 1, bx] + fx * q[:, by + 1, bx + 1]
            scores[:, y, x] = (5 - fy) * h0 + fy * h1
    # RTL horizontal/score widths are sufficient: [-640,635]/[-3200,3175].
    labels = np.argmax(scores, axis=0).astype(np.uint8)
    tied = np.count_nonzero(scores == scores.max(axis=0), axis=0) > 1
    return labels, scores, tied


def export_rtl_integer_reference(output_dir, labels, float_labels, logits,
                                 placements, tile_stats, checkpoint_sha256):
    """Export independent software reference, never consuming FPGA readback."""
    output_dir = Path(output_dir)
    labels = np.ascontiguousarray(labels, dtype=np.uint8)
    logits = np.ascontiguousarray(logits, dtype=np.int8)
    np.save(output_dir / "int8_hw_prediction_rtl_integer.npy", labels)
    (output_dir / "int8_hw_prediction_rtl_integer.bin").write_bytes(labels.tobytes())
    np.save(output_dir / "int8_head_logits_tiles.npy", logits)
    # Each output beat is one pixel, with class0 at the lowest byte address.
    packed_logits = np.ascontiguousarray(logits.transpose(0, 2, 3, 1))
    (output_dir / "int8_head_logits_tiles_pixel_class.bin").write_bytes(packed_logits.tobytes())
    different = labels != float_labels
    np.save(output_dir / "rtl_integer_vs_float_mismatch_mask.npy", different)
    report = {
        "schema": "rtl_integer_bilinear_v1",
        "rtl_module": "up_logits_to_rows",
        "source": "software INT8 head logits before float scale/interpolation; no board data",
        "input_shape_per_tile": list(logits.shape[1:]),
        "logits_npy_layout": "tile,class,y4,x4; signed int8",
        "logits_bin_layout": "tile,y4,x4,class; signed int8 two's-complement bytes",
        "label_layout": "scene y,x row-major; uint8 IDs 0..C-1; no DDR pitch padding",
        "scene_shape": list(labels.shape),
        "axis": "base=min(position//5,2); fraction=position-5*base; position=0..15",
        "score_denominator_omitted": 25,
        "score_range": [-3200, 3175],
        "rounding": "none",
        "tie_break": "lowest class ID, strict greater-than replacement",
        "boundary": "interpolate full 16x16 first, then crop valid_height/valid_width",
        "float_reference_unchanged": "int8_hw_prediction.npy",
        "rtl_reference": "int8_hw_prediction_rtl_integer.npy",
        "checkpoint_sha256": checkpoint_sha256,
        "label_sha256": hashlib.sha256(labels.tobytes()).hexdigest(),
        "input_tiles_sha256": _sha256_file(output_dir / "pcie_input/scene_tiles_uint8.bin"),
        "float_label_mismatches": int(np.count_nonzero(different)),
        "tie_pixels": sum(t["tie_pixels"] for t in tile_stats),
        "mismatches_with_float_winner_tied": sum(t["mismatches_with_float_winner_tied"] for t in tile_stats),
        "mismatches_not_explained_by_ties": sum(t["mismatches_not_explained_by_ties"] for t in tile_stats),
        "tiles": [dict(p, **s) for p, s in zip(placements, tile_stats)],
    }
    (output_dir / "rtl_integer_reference.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    print("[RTL integer reference] pixels={} float_differences={} tie_explained={} other={}".format(
        labels.size, report["float_label_mismatches"],
        report["mismatches_with_float_winner_tied"], report["mismatches_not_explained_by_ties"]))
    print("[RTL integer reference] PCIe --expected-labels", output_dir / report["rtl_reference"])
    return report
...
def simulate_mamba_hsi_dense16_both(
    ...
    rtl_integer_canvas = np.zeros((full_h, full_w), dtype=np.uint8)
    rtl_tile_logits, rtl_tile_placements, rtl_tile_stats = [], [], []
    ...
    with torch.no_grad():
        for tile_index, block in enumerate(blocks, start=1):
            ...
            # Additional hardware-contract reference; original float path stays intact.
            scale_value = float(logits_scale.item())
            if not math.isfinite(scale_value) or scale_value <= 0:
                raise ValueError("RTL argmax assumes one finite positive logits scale")
            q_cpu = logits_int8.detach().cpu().numpy()
            if q_cpu.shape != (1, class_count, 4, 4):
                raise ValueError(f"Unexpected head logits shape: {q_cpu.shape}")
            rtl_labels, rtl_scores, rtl_ties = rtl_integer_bilinear_argmax(q_cpu[0])
            ...
            rtl_integer_canvas[rows, cols] = rtl_labels[:valid_height, :valid_width]
            rtl_tile_logits.append(q_cpu[0].astype(np.int8, copy=True))
            rtl_tile_placements.append(dict(
                tile=tile_index - 1, top=int(top), left=int(left),
                valid_height=int(valid_height), valid_width=int(valid_width),
                logits_scale=scale_value))
            float_tile_labels = int8_up.argmax(dim=1)[0].cpu().numpy()
            delta = rtl_labels[:valid_height, :valid_width] != float_tile_labels[:valid_height, :valid_width]
            float_winner_scores = np.take_along_axis(
                rtl_scores, float_tile_labels[None, :, :], axis=0)[0]
            explained = (rtl_ties & (float_winner_scores == rtl_scores.max(axis=0)))[:valid_height, :valid_width]
            rtl_tile_stats.append(dict(
                tie_pixels=int(np.count_nonzero(rtl_ties[:valid_height, :valid_width])),
                float_label_mismatches=int(np.count_nonzero(delta)),
                mismatches_with_float_winner_tied=int(np.count_nonzero(delta & explained)),
                mismatches_not_explained_by_ties=int(np.count_nonzero(delta & ~explained))))
    ...
    rtl_integer_report = export_rtl_integer_reference(
        output_dir, rtl_integer_canvas, predict_int8, np.stack(rtl_tile_logits),
        rtl_tile_placements, rtl_tile_stats, _sha256_file(model_path))
    ...
    result = {
        "rtl_integer_postprocess": {k: v for k, v in rtl_integer_report.items() if k != "tiles"},
        "numeric_config": numeric.to_dict(),
        ...
        "ssm_execution_mode": "exact-integer" if numeric.error_source == 'all' else "floating-counterfactual",
