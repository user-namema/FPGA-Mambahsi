...
def validate_args(args):
    ...
    if args.activation_scale_lr_multiplier <= 0 or args.dt_scale_lr_multiplier <= 0:
...
class WeightFakeQuant(nn.Module):
    ...
    def __init__(self, bit=8, all_positive=False):
        super().__init__()
...
def add_fpga_fake_quant_nodes(model, nbit=8, dt_output_bits=8, d_weight_bits=8):
    ...
    for layer in model.mamba:
        ...
        for _, branch in active_block_branches(layer):
            ...
            if core.use_D:
                core.d_weight_quant = WeightFakeQuant(d_weight_bits, all_positive=False)
...
def make_qat_optimizer(model, args, logger):
    ...
    learning_rates = {
        ...
        "weight_scales": args.lr,
        "d_parameters": args.lr,
        "weights_and_bn": args.lr,
    }
    groups = []
        ...
        groups.append(
            {
                ...
                "weight_decay": (
                    args.weight_decay if name == "weights_and_bn" else 0.0
                ),
                "name": name,
            }
        )
...
def run_seed(
    ...
    if (run_dir / "best_qat_foldaware.pth").exists():
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if epoch == 1 or epoch % args.eval_interval == 0 or epoch == args.max_epoch:
            _, visits, metrics = evaluate_model(
                model,
                val_loader,
                gt.shape,
                args.tile_size,
            ...
            logger.info(
                ...
                "val_mAcc=%.6f select_%s=%.6f BN_frozen=%s",
                seed,
                epoch,
                args.max_epoch,
                loss,
    ...
    result = {
        ...
        "optimizer": "Adam",
        "initial_lr": args.lr,
        "final_lr": float(optimizer.param_groups[-1]["lr"]),
...
def main():
    ...
    print(f"All QAT runs completed. Summary saved to: {output}")
