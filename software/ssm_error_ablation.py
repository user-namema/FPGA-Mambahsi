...
class SSMNumericConfig:
    ...
    readout_requantization: str = 'auto'
    ...
    def __post_init__(self):
        ...
        if self.readout_requantization not in ('auto', 'hardware', 'ideal'):
            raise ValueError('invalid readout_requantization')
        if self.readout_requantization == 'hardware' and self.error_source != 'all':
            raise ValueError('hardware readout requires integer SSM (error_source=all)')

    @property
    def resolved_readout_requantization(self):
        if self.readout_requantization == 'auto':
            return 'hardware' if self.error_source == 'all' else 'ideal'
        return self.readout_requantization
...
def add_numeric_arguments(parser, boundary_defaults=True):
    ...
    for name in defaults.to_dict():
        if name in ('rounding', 'error_source', 'coefficient_backend', 'readout_requantization'):
            choices = {'rounding': ('single','separate'), 'error_source': ('all','none','a-only','k-only','state-only','d-only'), 'coefficient_backend': ('exact','pwl'), 'readout_requantization': ('auto','hardware','ideal')}[name]
