...
class DTInputDiagnostics:
    """Read-only statistics on the actual low-rank dt slice, before saturation."""

    def __init__(self):
        self.rows = {}

    def observe(self, name, rank, accumulator, accumulator_scale, scale,
                bits, actual_preclamp, reference):
        acc = accumulator[..., :rank]
        physical = acc.double() * accumulator_scale.double()
        ref = reference[..., :rank].detach().double()
        current_max = (1 << (bits - 1)) - 1
        # A fixed multiplier of the saved scale, never a fitted test-set scale.
        wider = max(1.0, current_max / 127.0)
        candidates = [('current', bits, scale, actual_preclamp[..., :rank]),
                      ('int8_same_scale', 8, scale, actual_preclamp[..., :rank])]
        wide_scale = scale * wider
        mult, shift = get_m_int_shift(accumulator_scale / wide_scale, q_bits=16)
        wide_raw = apply_multiplier_shift(acc, mult.to(acc.device), shift)
        candidates.append(('int8_wider_scale', 8, wide_scale, wide_raw))
        for variant, width, candidate_scale, raw in candidates:
            low, high = -(1 << (width - 1)), (1 << (width - 1)) - 1
            clipped = raw.clamp(low, high)
            fixed = clipped.double() * candidate_scale.double()
            local_error = fixed - physical
            ref_error = fixed - ref
            key = (name, variant)
            row = self.rows.setdefault(key, dict(core=name, variant=variant,
                bits=width, scale=float(candidate_scale), dt_rank=rank, count=0,
                tile_calls=0, clipped_count=0, endpoint_count=0,
                raw_code_min=int(raw.min()), raw_code_max=int(raw.max()),
                hardware_prequant_min=float(physical.min()), hardware_prequant_max=float(physical.max()),
                reference_min=float(ref.min()), reference_max=float(ref.max()),
                abs_error_sum=0.0, squared_error_sum=0.0, reference_energy_sum=0.0,
                max_abs_error=0.0, ref_abs_error_sum=0.0, ref_max_abs_error=0.0))
            row['count'] += raw.numel()
            row['tile_calls'] += 1
            row['clipped_count'] += int(((raw < low) | (raw > high)).sum())
            row['endpoint_count'] += int(((clipped == low) | (clipped == high)).sum())
            for label, value in [('raw_code_min', int(raw.min())), ('hardware_prequant_min', float(physical.min())), ('reference_min', float(ref.min()))]:
                row[label] = min(row[label], value)
            for label, value in [('raw_code_max', int(raw.max())), ('hardware_prequant_max', float(physical.max())), ('reference_max', float(ref.max()))]:
                row[label] = max(row[label], value)
            row['abs_error_sum'] += float(local_error.abs().sum())
            row['squared_error_sum'] += float(local_error.square().sum())
            row['reference_energy_sum'] += float(physical.square().sum())
            row['max_abs_error'] = max(row['max_abs_error'], float(local_error.abs().max()))
            row['ref_abs_error_sum'] += float(ref_error.abs().sum())
            row['ref_max_abs_error'] = max(row['ref_max_abs_error'], float(ref_error.abs().max()))

    def write(self, directory, split):
        import csv
        rows = []
        for row in self.rows.values():
            r = dict(row)
            r['clipped_pct'] = 100 * r['clipped_count'] / r['count']
            r['endpoint_pct'] = 100 * r['endpoint_count'] / r['count']
            r['local_MAE'] = r['abs_error_sum'] / r['count']
            r['local_RMSE'] = math.sqrt(r['squared_error_sum'] / r['count'])
            r['local_NRMSE'] = (math.sqrt(r['squared_error_sum'] / r['reference_energy_sum'])
                                if r['reference_energy_sum'] else None)
            r['vs_QAT_reference_MAE'] = r['ref_abs_error_sum'] / r['count']
            rows.append(r)
        path = Path(directory)
        if rows:
            with (path / 'dt_input_diagnostics.csv').open('w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=list(rows[0]))
                writer.writeheader()
                writer.writerows(rows)
        (path / 'dt_input_diagnostics.json').write_text(json.dumps(dict(
            split=split, scope='Actual dt_rank slice in selected split tiles; includes background/padding and all internal tokens, not a labeled-pixel statistic',
            counterfactual='INT8 candidates are read-only local requantization probes; never propagated to model output; wider scale is saved_scale * max(1, current_positive_qmax/127)',
            local_error_reference='Same hardware INT32 accumulator times accumulator scale before output requantization',
...
def sim_layer_int8(
    ...
    diagnostic = getattr(module, '_dt_input_diagnostic', None)
    if diagnostic is not None:
        observer, core, rank = diagnostic
        observer.observe(core, rank, accumulator, accumulator_scale, s_out,
                         output_bits, requantized, ref_val)
...
def sim_layer_requant(
    ...
    module_view._dt_input_diagnostic = dt_input_diagnostic
...
def simulate_mamba_core_int8(
    ...
    x_proj_for_dt, _, _ = sim_layer_requant(
        ...
        dt_input_diagnostic=((m._dt_input_observer, name_prefix, dt_rank)
                             if getattr(m, '_dt_input_observer', None) is not None else None),
    )
...
def simulate_mamba_hsi_dense16_both(
    ...
    dt_input_observer = DTInputDiagnostics() if report_dt_inputs else None
    captured_tiles = []
        ...
        for tile_index, block in enumerate(blocks, start=1):
            ...
            for module in net.modules():
                if hasattr(module, 'A_log_shared'):
                    ...
                    module._dt_input_observer = dt_input_observer if selected else None
    ...
    if dt_input_observer is not None:
        dt_input_observer.write(output_dir, ssm_analysis_split)
        print('[DT input diagnostics] Actual dt_rank slice: ' + str(output_dir / 'dt_input_diagnostics.csv'))
    result = {
        "numeric_config": numeric.to_dict(),
...
def parse_args():
    ...
    parser.add_argument('--report-dt-inputs', action='store_true',
                        help='Read-only current/INT8 saturation and error probes on actual dt_proj input slices')
...
def main():
    ...
    simulate_mamba_hsi_dense16_both(
        ...
        report_dt_inputs=args.report_dt_inputs,
